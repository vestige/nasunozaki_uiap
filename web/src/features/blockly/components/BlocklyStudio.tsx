  ...
  return (
    ...
    >
      ...
      <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1fr)_18rem]">
        <div className="lg:col-span-2 tabs tabs-lift after:hidden" role="tablist" aria-label="Blocklyと配線ガイドの表示切り替え"><button type="button" role="tab" aria-selected={activeEditorTab === "blocks"} className={`tab ${activeEditorTab === "blocks" ? "tab-active" : ""}`} onClick={() => setActiveEditorTab("blocks")}>ブロックプログラミング</button>{tactSwitchExtension.data ? <button type="button" role="tab" aria-selected={activeEditorTab === "wiring"} className={`tab ${activeEditorTab === "wiring" ? "tab-active" : ""}`} onClick={() => setActiveEditorTab("wiring")}>タクトスイッチ</button> : <button type="button" className="tab" onClick={() => { client.setQueryData(queryKeys.tactSwitchExtension, true); workspaceRef.current?.updateToolbox(tactSwitchToolbox); if (workspaceRef.current) saveBlocklyWorkspace(window.localStorage, Blockly.serialization.workspaces.save(workspaceRef.current), true); setActiveEditorTab("wiring"); }}>＋ タクトスイッチを追加</button>}</div>
        {tactSwitchExtension.data && activeEditorTab === "wiring" && <div className="-mt-5 lg:col-span-2 rounded-b-box rounded-tr-box border-2 border-base-300 bg-base-100 p-4" role="tabpanel"><div className="mb-3 flex items-center justify-between"><p className="font-black">タクトスイッチの配線</p><button type="button" className="btn btn-circle btn-ghost btn-sm" aria-label="タクトスイッチを外す" data-tooltip="タクトスイッチを外す" onClick={() => { if (!window.confirm("タクトスイッチ拡張を外します。すでに置いたブロックは消えません。続けますか？")) return; client.setQueryData(queryKeys.tactSwitchExtension, false); client.setQueryData(queryKeys.simulatorButton, false); workspaceRef.current?.updateToolbox(uiapToolbox); if (workspaceRef.current) saveBlocklyWorkspace(window.localStorage, Blockly.serialization.workspaces.save(workspaceRef.current), false); setActiveEditorTab("blocks"); }}>×</button></div><TactSwitchWiringGuide /></div>}
        <div className={`-mt-5 lg:col-span-2 rounded-b-box rounded-tr-box border-2 border-base-300 bg-base-100 p-4 ${tactSwitchExtension.data && activeEditorTab === "wiring" ? "hidden" : ""}`}><div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1fr)_18rem]"><div ref={mountWorkspace} className="blockly-workspace h-[38rem] w-full min-w-0 max-w-full overflow-hidden rounded-box border-2 border-neutral bg-white shadow-xl" aria-label="ブロックプログラミング編集エリア" /><LedSimulator ledOn={led.data} buttonPressed={button.data} extensionEnabled={tactSwitchExtension.data} instructions={program.data} onButtonChange={(pressed) => client.setQueryData(queryKeys.simulatorButton, pressed)} /></div></div>
      </div>
    </section>
  );
}
