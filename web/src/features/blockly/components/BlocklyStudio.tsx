  ...
  return (
    ...
    >
      ...
      <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1fr)_18rem]">
        ...
        {tactSwitchExtension.data && activeEditorTab === "wiring" && <div className="lg:col-span-2 rounded-box border-2 border-base-300 bg-base-100 p-4" role="tabpanel"><div className="mb-3 flex items-center justify-between"><p className="font-black">タクトスイッチの配線</p><button type="button" className="btn btn-ghost btn-sm" onClick={() => { if (!window.confirm("タクトスイッチ拡張を外します。すでに置いたブロックは消えません。続けますか？")) return; client.setQueryData(queryKeys.tactSwitchExtension, false); client.setQueryData(queryKeys.simulatorButton, false); workspaceRef.current?.updateToolbox(uiapToolbox); if (workspaceRef.current) saveBlocklyWorkspace(window.localStorage, Blockly.serialization.workspaces.save(workspaceRef.current), false); setActiveEditorTab("blocks"); }}>タクトスイッチを外す</button></div><TactSwitchWiringGuide /></div>}
        <div className={`lg:col-span-2 rounded-box border-2 border-base-300 bg-base-100 p-4 ${tactSwitchExtension.data && activeEditorTab === "wiring" ? "hidden" : ""}`}><div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1fr)_18rem]"><div ref={mountWorkspace} className="blockly-workspace h-[34rem] w-full min-w-0 max-w-full overflow-hidden rounded-box border-2 border-neutral bg-white shadow-xl" aria-label="ブロックプログラミング編集エリア" /><LedSimulator ledOn={led.data} buttonPressed={button.data} extensionEnabled={tactSwitchExtension.data} instructions={program.data} onButtonChange={(pressed) => client.setQueryData(queryKeys.simulatorButton, pressed)} /></div></div>
      </div>
    </section>
  );
}
