...
続いて、`preflight → 現在値照合 → erase → 全0xFF確認 → 元データwrite → 完全一致verify`を1つのtransactionとして実装した。erase後の確認失敗時にも元データの復旧を試み、復旧できたかを型付きエラーで返す。現在値が退避内容と異なる場合にeraseを呼ばないことを含め、自動テストで固定した。まだ実機送信層や画面には接続していない。
