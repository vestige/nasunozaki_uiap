...
修正版では `0x1FFFF7C4`から `0x00310510`を取得し、完了確認は1回だった。次はCH32V003用書き込みstubを移植し、packet長、64バイト境界、対象flash範囲、実行マジック位置を実機なしの自動テストで固定する。この段階では生成したpacketをWebHIDへ送らない。
